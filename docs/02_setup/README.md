...
1. [Azure OpenAI](https://ms.portal.azure.com/#create/Microsoft.CognitiveServicesOpenAI), with 3 model deployments, one of the **gpt-4o-realtime-preview** model, a regular **gpt-4o-mini** model and a **text-embedding-ada-002** model. Try to provide as much quota as possible to the realtime-preview to allow for voice interactions, while minimizing quota for other models.
1. [Optional] Train an intent_detection model with a SLM using Azure AI Studio. Check [the training data](../../voice_agent/intent_detection_model)

### Setting up the environment

The app needs to know which service endpoints to use for the Azure OpenAI and Azure AI Search. The following variables can be set as environment variables, or you can create a ".env" file in the "app/backend/" directory with this content. A sample file, **.env.sample** has been provided. 
...
# These can be left alone if using gpt-4o-mini model
...
There are multiple options for running the code in this repo. Please choose one of the following, **GitHub Codespaces**, **VS Code Dev Containers**, or **Local environment**. 
...
1. Ensure env variables are set per [Setting up the environment](#setting-up-the-environment)

### Running the app
You can choose to runn the app locally on your machine or deploy the app to Azure and run in a container.
